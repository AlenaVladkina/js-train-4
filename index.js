...
function checkIfStartsWith(mainStr, subStr) {
  ...
  return mainStr.startsWith(subStr) ? true : false;
}
...
function checkIfEndsWith(mainStr, subStr) {
  ...
  return mainStr.endsWith(subStr) ? true : false;
}
...
function compareCharCodeAtAndCodePointAt(string, index) {
  ...
  // Використовуємо метод charCodeAt(), щоб отримати код символа з рядка на певній позиції, та запишемо результат в змінну 
